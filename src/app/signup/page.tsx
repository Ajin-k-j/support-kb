  ...
  const onSubmit = async (data: SignupForm) => {
    try {
      ...
      switch (err.code) {
        ...
        case 'auth/weak-password': friendlyMessage = 'Password must be at least 6 characters.'; break;
  ...
  return (
    <>
      <style>{`
        ...
        .auth-tabs { display: flex; background: #f3f4f6; border-radius: 12px; padding: 4px; gap: 4px; margin-bottom: 1.5rem; }
        .auth-tab {
        ...
        .auth-tab.active { background: #ffffff; color: #4f46e5; box-shadow: 0 1px 4px rgba(0,0,0,0.10); }
        ...
        .auth-btn-primary {
          ...
          box-shadow: 0 4px 14px rgba(79,70,229,0.30); margin-bottom: 0.5rem;
        }
        ...
        .auth-divider { display: flex; align-items: center; gap: 10px; color: #9ca3af; font-size: 0.75rem; margin: 1rem 0; }
        .auth-divider::before, .auth-divider::after { content: ''; flex: 1; height: 1px; background: #e5e7eb; }
        .auth-error {
          display: flex; align-items: flex-start; gap: 8px;
          background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px;
          color: #b91c1c; font-size: 0.8rem; padding: 10px 12px;
        ...
        .auth-badge {
          ...
          border-radius: 99px; padding: 3px 10px; font-size: 0.68rem; font-weight: 700;
          color: #4f46e5; letter-spacing: 0.05em; text-transform: uppercase; margin-bottom: 0.75rem;
        }
      ...
      <div className="auth-page-root">
        ...
        <div className="auth-card">
          ...
          <p className="auth-sub">Create your workspace account to start managing support tickets.</p>

          <div className="auth-tabs">
            <button id="tab-email-signup" className={`auth-tab${activeTab === 'email' ? ' active' : ''}`} onClick={() => { setActiveTab('email'); setError(null); }}>Email</button>
            <button id="tab-google-signup" className={`auth-tab${activeTab === 'google' ? ' active' : ''}`} onClick={() => { setActiveTab('google'); setError(null); }}>Google</button>
          </div>
          ...
          {activeTab === 'email' && (
            <form onSubmit={handleSubmit(onSubmit)} noValidate>
              <div className="auth-form-group">
                ...
                <input id="signup-name" {...register('displayName')} type="text" className={`auth-input${errors.displayName ? ' error-field' : ''}`} placeholder="Jane Smith" autoComplete="name" />
                {errors.displayName && <div className="auth-field-error">{errors.displayName.message}</div>}
              </div>
              <div className="auth-form-group">
                <label className="auth-label" htmlFor="signup-email">Work Email</label>
                <input id="signup-email" {...register('email')} type="email" className={`auth-input${errors.email ? ' error-field' : ''}`} placeholder="you@company.com" autoComplete="email" />
                {errors.email && <div className="auth-field-error">{errors.email.message}</div>}
              </div>
              <div className="auth-form-group">
                <label className="auth-label" htmlFor="signup-password">Password</label>
                <input id="signup-password" {...register('password')} type="password" className={`auth-input${errors.password ? ' error-field' : ''}`} placeholder="Min. 6 characters" autoComplete="new-password" />
                {errors.password && <div className="auth-field-error">{errors.password.message}</div>}
              </div>
              <button id="btn-email-signup" type="submit" className="auth-btn auth-btn-primary" disabled={isEmailLoading}>
                {isEmailLoading ? <><Loader /><span>Creating account…</span></> : 'Create Account'}
              </button>
            </form>
          )}
          ...
          {activeTab === 'google' && (
            <div>
              <div className="auth-divider">Sign up with your Google account</div>
              <button id="btn-google-signup" type="button" className="auth-btn auth-btn-google" onClick={handleGoogleSignUp} disabled={isGoogleLoading}>
